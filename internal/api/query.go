...
func (h *Handler) executeSessionQuery(sessionDir string, meta sessionMetadata, req queryRequest) (map[string]any, error) {
	...
	if shouldRetryWithHeuristicPlan(plannerWarnings, execResult) {
		repairedPlan := buildQueryPlan(snapshot, req.Question)
...
func (h *Handler) buildQueryPlanWithFallback(settings modelSettings, snapshot schemaSnapshot, question string) (queryPlan, []string) {
	heuristicPlan := buildQueryPlan(snapshot, question)
	if !llmEnabled(settings) {
		return heuristicPlan, nil
	}
	if len(snapshot.Tables) == 0 {
		return heuristicPlan, []string{"LLM SQL generation skipped because no imported tables are available."}
	}
	...
	if err != nil {
		return heuristicPlan, []string{"LLM SQL generation failed; using heuristic query planner."}
	}
	if !isSafeReadOnlySQL(llmResp.SQL) {
		return heuristicPlan, []string{"LLM returned an unsafe or unsupported SQL statement; using heuristic query planner."}
	}
