...
func (h *Handler) buildQueryPlanWithFallback(settings modelSettings, databasePath string, snapshot schemaSnapshot, question string) (queryPlan, []string) {
	...
	table := chooseLLMSourceTable(snapshot, llmResp, heuristicPlan)
	...
	}, heuristicPlan.DimensionColumn)
	metricColumn := chooseLLMColumn(llmResp.MetricColumn, table, isMetricColumn, heuristicPlan.MetricColumn)
	timeColumn := chooseLLMColumn(llmResp.TimeColumn, table, isTimeColumn, heuristicPlan.TimeColumn)
...
func chooseLLMSourceTable(snapshot schemaSnapshot, resp llmSQLResponse, fallback queryPlan) tableSchema {
	if table := findTableByName(snapshot, strings.TrimSpace(resp.SourceTable)); table.TableName != "" {
		return table
	}
	if table := findTableByName(snapshot, fallback.SourceTable); table.TableName != "" {
		return table
	}
	if len(snapshot.Tables) > 0 {
