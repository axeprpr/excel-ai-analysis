...
func (h *Handler) executeSessionQuery(sessionDir string, meta sessionMetadata, req queryRequest) (map[string]any, error) {
	...
	if shouldBuildAnalysisReport(req.Question) {
		if report := h.buildAnalysisReport(meta, settings, snapshot, req.Question, chartMode); len(report) > 0 {
...
func (h *Handler) buildQueryPlanWithFallback(settings modelSettings, databasePath string, snapshot schemaSnapshot, question string) (queryPlan, []string) {
	if !llmEnabled(settings) {
		return h.buildHeuristicQueryPlan(settings, snapshot, question)
	}
	...
	if llmResp.Refuse {
		refusalPlan := buildMinimalLLMRefusalPlan(snapshot, question)
		refusalPlan.Mode = "refuse"
		refusalPlan.SQL = ""
		refusalPlan.SelectionReason = strings.TrimSpace(llmResp.Reason)
		return refusalPlan, []string{"The configured LLM declined to answer because the request was ambiguous or unsupported."}
	}
	safeSQL, validationErr := sanitizeReadOnlySQL(llmResp.SQL)
	if validationErr != nil {
		heuristicPlan, heuristicWarnings := h.buildHeuristicQueryPlan(settings, snapshot, question)
	...
	metricColumn := chooseLLMColumn(llmResp.MetricColumn, table, isMetricColumn, "")
	timeColumn := chooseLLMColumn(llmResp.TimeColumn, table, isTimeColumn, "")
	...
	return queryPlan{
		...
		SelectionReason:    "selected by LLM-generated SQL using schema headers, sample rows, and lightweight column statistics",
		DimensionColumn:    dimensionColumn,
		MetricColumn:       metricColumn,
		TimeColumn:         timeColumn,
		SelectedColumns:    selectedColumnsForPlan(mode, dimensionColumn, metricColumn, timeColumn),
		Filters:            []string{},
		Question:           question,
		ChartType:          chartType,
		Mode:               mode,
...
func buildLLMSchemaFacts(databasePath string, snapshot schemaSnapshot) []llmTableFact {
	catalog, err := readSchemaCatalogFromDatabase(databasePath)
	if err != nil || len(catalog) == 0 {
		facts := make([]llmTableFact, 0, len(snapshot.Tables))
		for _, table := range snapshot.Tables {
			facts = append(facts, llmTableFact{
				TableName:   table.TableName,
				SourceFile:  table.SourceFile,
				SourceSheet: table.SourceSheet,
				Columns:     buildFallbackLLMColumnFacts(table.Columns),
			})
		}
		return facts
	}

	facts := make([]llmTableFact, 0, len(catalog))
	for _, table := range catalog {
		facts = append(facts, llmTableFact{
			...
			Columns:     buildLLMColumnFacts(table.Columns, table.PreviewRows),
...
func buildFallbackLLMColumnFacts(columns []schemaColumn) []llmColumnFact {
	facts := make([]llmColumnFact, 0, len(columns))
	for _, column := range columns {
		facts = append(facts, llmColumnFact{
			Name:     column.Name,
			Type:     column.Type,
...
func buildLLMColumnFacts(columns []schemaColumn, previewRows []map[string]any) []llmColumnFact {
	facts := make([]llmColumnFact, 0, len(columns))
	for _, column := range columns {
		...
		for _, row := range previewRows {
			value := strings.TrimSpace(fmt.Sprint(row[column.Name]))
			if value == "" || strings.EqualFold(value, "<nil>") || strings.EqualFold(value, "null") {
				emptyValueCount++
				continue
			}
			if len(samples) < 3 {
		...
		facts = append(facts, llmColumnFact{
			...
			Type:            column.Type,
			Semantic:        column.Semantic,
			NonNullSamples:  samples,
			ApproxDistinct:  len(distinct),
...
func chooseLLMSourceTable(snapshot schemaSnapshot, resp llmSQLResponse) tableSchema {
	if table := findTableByName(snapshot, strings.TrimSpace(resp.SourceTable)); table.TableName != "" {
		return table
	}
	if len(snapshot.Tables) > 0 {
...
func chooseLLMColumn(name string, table tableSchema, predicate func(schemaColumn) bool, fallback string) string {
	name = strings.TrimSpace(name)
	for _, column := range table.Columns {
		if column.Name == name && predicate(column) {
			return column.Name
		}
	}
	return fallback
...
func normalizeLLMChartType(chartType string) string {
	switch strings.ToLower(strings.TrimSpace(chartType)) {
	case "table", "bar", "line", "pie":
		return strings.ToLower(strings.TrimSpace(chartType))
	default:
		return ""
	}
}
...
func shouldBuildAnalysisReport(question string) bool {
	...
	return containsAny(normalized, []string{
		"分析", "analysis", "overview", "dashboard", "report", "总览",
	})
}
...
func buildAnalysisQuestions(snapshot schemaSnapshot) []string {
	...
	if primaryCategory != "" {
		questions = append(questions, "按"+primaryCategory+"查看分布饼图")
	}
	if secondaryDimension != "" && secondaryDimension != primaryCategory {
		questions = append(questions, "按"+secondaryDimension+"查看对比柱状图")
	}
	if timeColumn != "" {
		questions = append(questions, "按"+timeColumn+"查看趋势折线图")
	}
	if primaryCategory == "" && secondaryDimension == "" {
		questions = append(questions, "按维度查看分布饼图")
	}
	return questions
}

func buildMinimalLLMRefusalPlan(snapshot schemaSnapshot, question string) queryPlan {
	plan := queryPlan{
		Question:        question,
		Mode:            "refuse",
		ChartType:       "table",
	...
	if len(snapshot.Tables) > 0 {
		plan.SourceTable = snapshot.Tables[0].TableName
		plan.SourceFile = snapshot.Tables[0].SourceFile
		plan.SourceSheet = snapshot.Tables[0].SourceSheet
		plan.CandidateTables = []string{snapshot.Tables[0].TableName}
	}
	return plan
}

func inferModeFromSQL(sql string) string {
	lower := strings.ToLower(sql)
	switch {
	case strings.Contains(lower, "share_value"):
		return "share"
	case strings.Contains(lower, "time_bucket"), strings.Contains(lower, "compare_period"):
		return "trend"
	case strings.Contains(lower, "count(*)"):
		return "count"
	case strings.Contains(lower, "sum("), strings.Contains(lower, "avg("), strings.Contains(lower, "min("), strings.Contains(lower, "max("):
